...
namespace ReportsManagementSystem.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult BookingData(string p_AccommodationSearch, string P_BookingSource, int? P_BookingStatusId, DateTime? P_StartDate, DateTime? P_EndDate, string P_AccommodationId_CHAR, int searchfilter,int? p_ReportGroupId,int? p_ReportSubGroupId, int P_Entity, int P_Query)
        {
            ...
            //ShowBookingdata
            if (searchfilter == 1 && !string.IsNullOrEmpty(P_AccommodationId_CHAR) && P_BookingStartDate.HasValue && P_BookingEndDate.HasValue && (!string.IsNullOrEmpty(P_BookingSource) || P_BookingStatusId > 0))
            {
                var bookingData = _context.bookingsDatas.FromSqlRaw("EXECUTE SP_ReportsManagement @P_Entity={0},@P_Query={1},@P_BookingStartDate={2},@P_BookingEndDate={3},@P_BookingSource={4},@P_AccommodationId_CHAR={5},@P_BookingStatusId={6}",
                    101, 1, P_BookingStartDate.Value, P_BookingEndDate.Value, P_BookingSource, P_AccommodationId_CHAR, P_BookingStatusId)
               .ToList();
            ...
            else if (searchfilter == 2 && !string.IsNullOrEmpty(P_AccommodationId_CHAR) && P_CheckInStartDate.HasValue && P_CheckInEndDate.HasValue && (!string.IsNullOrEmpty(P_BookingSource) || P_BookingStatusId > 0))
            {
                var bookingData = _context.bookingsDatas.FromSqlRaw("EXECUTE SP_ReportsManagement @P_Entity={0},@P_Query={1},@P_CheckInStartDate={2},@P_CheckInEndDate={3},@P_BookingSource={4},@P_AccommodationId_CHAR={5},@P_BookingStatusId={6}",
                    101, 1, P_CheckInStartDate.Value, P_CheckInEndDate.Value, P_BookingSource, P_AccommodationId_CHAR, P_BookingStatusId)
               .ToList();
                ViewBag.TotalBookings = bookingData.Any() ? bookingData.First().TotalBookings : 0;
                ViewBag.TotalRevenue = bookingData.Any() ? bookingData.First().TotalRevenue : 0;
                ViewBag.totalnight = bookingData.Any() ? bookingData.First().TotalNights : 0;
            ...
            else if (searchfilter == 3 && !string.IsNullOrEmpty(P_AccommodationId_CHAR) && P_CheckOutStartDate.HasValue && P_CheckOutEndDate.HasValue && (!string.IsNullOrEmpty(P_BookingSource) || P_BookingStatusId > 0))
            {
                var bookingData = _context.bookingsDatas.FromSqlRaw("EXECUTE SP_ReportsManagement @P_Entity={0},@P_Query={1},@P_CheckOutStartDate={2},@P_CheckOutEndDate={3},@P_BookingSource={4},@P_AccommodationId_CHAR={5},@P_BookingStatusId={6}",
                    101, 1, P_CheckOutStartDate.Value, P_CheckOutEndDate.Value, P_BookingSource, P_AccommodationId_CHAR, P_BookingStatusId)
               .ToList();
                ViewBag.TotalBookings = bookingData.Any() ? bookingData.First().TotalBookings : 0;
                ViewBag.TotalRevenue = bookingData.Any() ? bookingData.First().TotalRevenue : 0;
                ViewBag.totalnight = bookingData.Any() ? bookingData.First().TotalNights : 0;
        ...
        public IActionResult CreateSubGroup(ReportsSubGroups model)
        {
            ...
            return RedirectToAction("CreateSubGroup", "Home");
        }
        [HttpGet]
        ...
        public async Task<IActionResult> GroupsReport(int p_ReportGroupId, int p_ReportSubGroupId)
        {
            ...
            var result = showgroup.Select(x => new {
                reportGroupId = x.ReportGroupId,
                reportGroupName = x.ReportGroupName,
                reportSubGroupId = x.ReportSubGroupId,
                reportSubGroupName = x.ReportSubGroupName
            });
